...
Heston_get_init<- function(
  ...
  if (!is.null(initial_estimate)) {
    ...
    if (!check_initial_estimate(initial_estimate, verbose = verbose, return_value = FALSE)) {
      ...
      initial_estimate <- try(
        par2parUncon_heston(
          parUncon2par_heston(initial_estimate, controls), controls, use_parameter_labels = TRUE
        ), silent = TRUE
      )
  ...
  initial_heuristic <- function(data, states, positive_mu) {
    ...
    for (s in seq_len(states)) {
      ...
      # theta_estimate
      theta_s <- mean(cluster_s, na.rm = TRUE)
      theta_est[s] <- max(theta_s, 1e-6) # 确保正数
      
     
      if (states == 2) {
        kappa_est[s] <- ifelse(s == 1, 3.0, 0.5) 
      } else {
        # other intitial guesses
        kappa_est[s] <- 1.0 
      }
  
      sigma_s_base <- sd(cluster_s, na.rm = TRUE)
      sigma_est[s] <- max(sigma_s_base * 2, 0.05)
    }
