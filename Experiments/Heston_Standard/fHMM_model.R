...
summary.fHMM_model <- function(object, alpha = 0.05, ...) {
  ...
  sdds <- parUncon2par_heston(object$estimate, object$data$controls)$sdds
...
coef.fHMM_model <- function(object, alpha = 0.05, digits = 2, ...) {
  ...
  if (object$data$controls$simulated) {
    true <- par2parCon_heston(object$data$true_parameters, object$data$controls)
...
predict.fHMM_model <- function(object, ahead = 5, alpha = 0.05, ...) {
  ...
  ### extract parameters
  par <- parUncon2par_heston(object$estimate, object$data$controls)
