...
#' 
#' @param controls
#' Either a \code{list} or an object of class \code{Heston_controls}.
#'
...
#' 
#' @return
#' An object of class \code{Heston_controls}, which is a \code{list} that contains
...
Heston_set_controls <- function(
  ...
  if (!is.list(controls)) {
    stop(
      "Input 'controls' must be a list or an object of class 'Heston_controls'.", 
      call. = FALSE
    )
  }
...
validate_controls <- function(controls) {
  ...
  structure(controls, class = c("Heston_controls", "list"))
}

#' @rdname Heston_set_controls
#' @exportS3Method 

print.Heston_controls <- function(x, ...) {
  cat("Heston controls:\n")
...
summary.Heston_controls <- function(object, ...) {
  if (!inherits(object, "Heston_controls")) {
    stop("Not an object of class 'Heston_controls'.", call. = FALSE)
  }
