...
#'
#' @description
#' This function constructs an object of class \code{Heston_data}, which contains
...
#' @param controls
#' The \code{Heston_controls} object.
#' @param true_parameters
#' The \code{Heston_parameters} object in the simulated case.
#' 
#' @return 
#' An object of class \code{Heston_data}, which is a \code{list} containing 
...
#'
#' @description
#' This function simulates or reads financial data for the \{Heston\} package.
#'
#' @param controls
#' An object of class \code{Heston_controls}.
#' @param true_parameters
#' An object of class \code{Heston_parameters}, used as simulation parameters.
...
#'
#' @return
#' An object of class \code{\link{Heston_data}}.
#'
#' @examples
...
prepare_data <- function(controls, true_parameters = NULL, seed = NULL) {
  ...
  ### check inputs
  if (!inherits(controls,"Heston_controls")) {
    stop("'controls' is not of class 'Heston_controls'.", call. = FALSE)
  }
  
  ### process data
  if (controls[["simulated"]]) {
    if (is.null(true_parameters)) {
      true_parameters <- Heston_parameters(controls, seed = seed)
    }
    if (!inherits(true_parameters,"Heston_parameters")) {
      stop("'true_parameters' is not of class 'Heston_parameters'.", 
           call. = FALSE)
    }
    data <- simulate_hmm(
...
#' @rdname Heston_data
#' @param x
#' An object of class \code{Heston_data}.
#' @param ...
#' Currently not used.
#' @exportS3Method 

print.Heston_data <- function(x, ...) {
  cat("Heston", ifelse(x$controls$simulated, "simulated", "empirical"), "data\n")
  return(invisible(x))
}
...
summary.Heston_data <- function(object, ...) {
...
print.summary.Heston_data <- function(x, ...) {
  cat("Summary of Heston", ifelse(x$simulated, "simulated", "empirical"), 
      "data\n")
